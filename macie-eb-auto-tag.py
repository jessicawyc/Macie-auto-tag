#针对从eventbridge过来的单条macie-custom-identifier-finding 进行分析后对文件打tag
import json
import boto3
import os
s3 = boto3.client('s3')
...
datadic={"CREDIT_CARD_NUMBER":3,"USA_SOCIAL_SECURITY_NUMBER":1,"NAME":2,"ChineseID":4,"PHONE_NUMBER":2,"AWS_CREDENTIALS":2,"ChineseID-nokeyword":3}
def taglevel(typelist,oldlevel):
    ...
    for i in range(len(typelist)):
        if (typelist[i] in datadic.keys()):
            ...
            print(str(typelist[i])+'未找到其定义的级别')
    newlevel=max(oldlevel,max(levels))
    print('发现新级别为:'+str(newlevel))
    return(newlevel)
#给object打上标签
...
def lambda_handler(event, context):
    ...
    s3tag = event["detail"]["resourcesAffected"]["s3Bucket"]["tags"] 
